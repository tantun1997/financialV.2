...
// นำเข้าคลาส Controller ที่ถูกต้อง

class PMaintenance extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $E_TYPE = DB::table('E_TYPE')->where('TYPE_NUMBER', 1)->get();
        ...
        // ส่งข้อมูลไปยัง view
        return view('maintenances.index', compact('E_PLAN', 'E_TYPE', 'E_BUDGET', 'E_USAGE_STATUS', 'E_CLOSE_PLAN'));
    }
    ...
    {
        // ดึงค่าปัจจุบันของ 'EQUP_USED' จากฐานข้อมูล
        $currentValue = DB::table('E_LIST_ITEM')
            ->where('EQUP_ID', $id)
            ->value('EQUP_USED');
    ...
    public function show(string $id)
    {
        ...
        return view('maintenances.show', compact('E_PLAN', 'EQUIP_LIST', 'EQUIP_STATUS', 'E_STATUS', 'E_TYPE', 'E_BUDGET', 'E_USAGE_STATUS', 'VW_EQUIPMENT'));
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        if (
            ...
            if ($typeId == 1) {
                ...
                return redirect()->route('repairs.show', $id);
