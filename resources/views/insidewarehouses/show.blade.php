...
@section('title', 'รายละเอียดแผนวัสดุในคลัง')
...
@section('contents')
   <style>
      ...
      .table td {
         ...
         /* ให้ตัดคำเมื่อเกินความกว้าง */
         white-space: nowrap;
         /* ให้ขึ้นบรรทัดใหม่เมื่อเกินความกว้าง */
         max-width: 300px;
   ...
   <div class="row">
      <div class="col-lg-12">
         <div style="display: flex; justify-content: space-between; align-items: center;">
            ...
            <button class="btn btn-primary" data-action="{{ route('insidewarehouses.update', $E_PLAN->PLAN_ID) }}" data-bs-target="#editModal" data-bs-toggle="modal" type="button">
               แก้ไขข้อมูล
            </button>
         </div>
               ...
               <div style="max-width: 100%; overflow-x: auto; overflow-y: auto;">
                  <table class="table-sm table">
                     <tr>
                        ...
                        <th>ปีงบประมาณ:</th>
                        <td>{{ $E_PLAN->BUDG_YEAR }}</td>
                        <th>ประเภทแผน:</th>
                        <td>{{ $E_PLAN->TYPE_NAME }}</td>
                        <th>ประเภทงบ:</th>
                        <td>{{ $E_PLAN->Budget_NAME }}</td>
                        <th>วงเงินรวม:</th>
                        <td>{{ number_format(round($E_PLAN->Total_Price, 2), 2) }} บาท</td>
                        <th>จำนวนวัสดุ:</th>
                        <td>{{ number_format(round($E_PLAN->Total_QTY, 2), 0) }}</td>
                     </tr>
                     <tr>
                        <th>หมวดพัสดุ:</th>
                        <td>{{ $E_PLAN->PRODCT_CAT_NAME }}</td>
                        <th>หน่วยงานที่เบิก:</th>
                        <td>{{ $E_PLAN->TCHN_LOCAT_NAME }}</td>
                        <th>วันที่สร้างแผน:</th>
                        <td>{{ \Carbon\Carbon::parse($E_PLAN->CREATE_DATE)->addYears(543)->format('d/m/Y') }}</td>
                     </tr>
                  </table>
               </div>
            </div>
         ...
         <div style="display: flex; justify-content: flex-end; align-items: center; margin-bottom: 10px;">
            <div style="display: flex; align-items: center;">
               ...
               @if (Auth::user()->id != '114000041')
                  <form id="approval-form" action="{{ route('insidewarehouses.approved', $E_PLAN->PLAN_ID) }}" method="POST">
                     @csrf
                     @method('PATCH')
               ...
               @if (Auth::user()->id == '114000041')
                  @if ($E_PLAN->REQ_APPROVED_ID == 1)
                     <form id="approval-form-finish" action="{{ route('insidewarehouses.approved', $E_PLAN->PLAN_ID) }}" method="POST">
                        @csrf
                        @method('PATCH')
                        <button class="btn btn-primary" type="button" style="white-space: nowrap;" onclick="checkCheckboxes(0)">
                           อนุมัติเสร็จสิ้น
                        </button>
                     </form>
                  @endif
                     ...
                     @foreach ($EQUIP_LIST as $item)
                        <tr>
                           <td style="text-align: center; vertical-align: middle;">
                              <form action="{{ route('insidewarehouses.update_equip_used', $item->PRODCT_ID) }}" method="POST">
                                 @csrf
                                 @method('PATCH')
                                 <input id="PRODCT_USED_{{ $item->PRODCT_ID }}" name="PRODCT_USED" type="checkbox" @if ($item->PRODCT_USED == 'Y') checked @endif>
                              </form>
                           </td>
                           <form class="equip-form" id="equip-form-{{ $item->PRODCT_ID }}" action="{{ route('insidewarehouses.update_equip', $item->PRODCT_ID) }}" method="POST">
                              @csrf
                              @method('PATCH')
                              <td>
                              ...
                              <td class="text-center">
                                 <div class="form-container">
                                    <div class="view-mode">{{ $item->PRODCT_QTY }}</div>
                                    <div class="edit-mode" style="display: none;">
                                       <div class="input-group">
                              ...
                              <td class="text-center">
                                 <div class="form-container">
                                    <div class="view-mode" data-input-name="PRODCT_CURRENT_PRICE">{{ $item->PRODCT_CURRENT_PRICE }}</div>
                                    <div class="edit-mode" style="display: none;">
                                       <div class="input-group">
                           ...
                           <td class="text-center align-middle" style="white-space: nowrap">
                              <div class="view-mode">
                                 <div class="btn-group mb-2" role="group">
                                    ...
                                    <form class="p-0" action="{{ route('insidewarehouses.destroy_equip', $item->PRODCT_ID) }}" method="POST" onsubmit="return confirm('Delete?')">
                                       @csrf
                                       @method('DELETE')
                                 ...
                                 <div>
                                    @if (Auth::user()->id == '114000041')
                                       <form id="approval-form-{{ $item->PRODCT_ID }}" action="{{ route('insidewarehouses.approved_list_product', $item->PRODCT_ID) }}" method="POST">
                                          @csrf
                                          @method('PATCH')

                                          @if ($item->REQ_APPROVED_ID == 10)
   ...
   @include('insidewarehouses.modal_edit')
   @include('insidewarehouses.modal_add_product')
                  ...
                  .then(data => {
                     if (data.success) {
                        ...
                        if (prodctCurrentPriceElement) {
                           prodctCurrentPriceElement.innerText = `${formData.get('PRODCT_CURRENT_PRICE')}`;
                        }
