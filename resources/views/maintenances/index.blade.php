    ...
    @section('contents')
       ...
       <div class="mb-3">
          @if ($E_CLOSE_PLAN->status == 'Y')
             <button type="button" class="btn btn-primary" id="addItemButton" data-bs-toggle="modal" data-bs-target="#addModal" data-action="{{ route('maintenances.store') }}">
                + เพิ่มรายการ
             </button>
          @endif
                   ...
                   <tr>
                      ...
                      <td class="text-center align-middle">
                         ...
                         @if ($item->USAGE_STATUS_ID == 1 && Auth::user()->id == '114000041')
                            <form id="approval-form-{{ $item->PLAN_ID }}" action="{{ route('maintenances.approved', $item->PLAN_ID) }}" method="POST">
                               @csrf
                               @method('PATCH')
          ...
          document.addEventListener('DOMContentLoaded', function() {
             ...
             document.querySelectorAll('.form-check-input').forEach((checkbox) => {
                checkbox.addEventListener('change', function() {
                   ...
                   fetch('{{ route('maintenances.close_plan') }}', {
                         method: 'POST',
                         headers: {
