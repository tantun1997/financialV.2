...
@section('title', 'รายละเอียดแผนจ้างเหมาบริการ')
...
@section('contents')
   ...
   <div class="row">
      <div class="col-lg-12">
         <div style="display: flex; justify-content: space-between; align-items: center;">
            ...
            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#editModal" data-action="{{ route('contractservices.update', $E_PLAN->PLAN_ID) }}">
               แก้ไขข้อมูล
            </button>
      ...
      <div class="col-lg-12">
         <div style="display: flex; justify-content: flex-end; align-items: center; margin-bottom: 10px;">
            <div style="display: flex; align-items: center;">
               <form action="{{ route('contractservices.addRow', $E_PLAN->PLAN_ID) }}" method="POST">
               ...
               <button onclick="generatePdf({{ $E_PLAN->PLAN_ID }}, 'contractservices')" class="btn btn-danger mr-1">
                  <i class="fa-duotone fa-file-pdf fa-lg"></i> PDF
               </button>
               @if ($E_PLAN->USAGE_STATUS_ID == 1 && Auth::user()->id != '114000041')
                  @if ($E_PLAN->REQ_APPROVED_ID == 0)
                     <form action="{{ route('contractservices.approved', $E_PLAN->PLAN_ID) }}" method="POST">
                        @csrf
                        @method('PATCH')
                  ...
                  @foreach ($EQUIP_LIST as $item)
                     <tr>
                        <td style="text-align: center; vertical-align: middle;">
                           <form action="{{ route('contractservices.update_equip_used', [$item->EQUP_ID, $E_PLAN->PLAN_ID]) }}" method="POST">
                              @csrf
                              @method('PATCH')
                        ...
                        <td style="text-align: left; white-space: nowrap; vertical-align: middle;">
                           <div class="form-container">
                              <form action="{{ route('contractservices.update_equip', $item->EQUP_ID) }}" method="POST" class="equip-form" id="equip-form-{{ $item->EQUP_ID }}">
                                 @csrf
                                 @method('PATCH')
                                 <div class="view-mode">
                        ...
                        <td class="text-center align-middle">
                           ...
                           <div class="view-mode">
                              <div class="btn-group" role="group">
                                 ...
                                 <form action="{{ route('contractservices.destroy_equip', $item->EQUP_ID) }}" method="POST" onsubmit="return confirm('Delete?')" class="p-0">
                                    @csrf
                                    @method('DELETE')
   ...
   @include('contractservices.modal_edit')
