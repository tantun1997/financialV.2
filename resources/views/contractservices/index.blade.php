    ...
    @section('title', 'แผนจ้างเหมาบริการ')
    ...
    @section('contents')
       ...
       <div class="mb-3">
          @if ($E_CLOSE_PLAN->status == 'Y')
             <button type="button" class="btn btn-primary" id="addItemButton" data-bs-toggle="modal" data-bs-target="#addModal" data-action="{{ route('contractservices.store') }}">
                + เพิ่มรายการ
             </button>
          @endif
       ...
       <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <hr />
          <a href="{{ route('export.data', ['type' => 'contractservices']) }}" class="btn btn-success mb-2">Export to
             Excel</a>
       </div>
                   ...
                   <tr>
                      ...
                      <td class="text-center align-middle">
                         <div class="btn-group" role="group">
                            <a href="{{ route('contractservices.show', $item->PLAN_ID) }}" type="button" class="btn btn-outline-primary btn-sm" style="white-space: nowrap;">ดูข้อมูล</a>
                            <form action="{{ route('contractservices.destroy', $item->PLAN_ID) }}" method="POST" onsubmit="return confirm('Delete?')" class="p-0">
                               @csrf
                               @method('DELETE')
                         ...
                         @if ($item->USAGE_STATUS_ID == 1 && Auth::user()->id == '114000041')
                            <form id="approval-form-{{ $item->PLAN_ID }}" action="{{ route('contractservices.approved', $item->PLAN_ID) }}" method="POST">
                               @csrf
                               @method('PATCH')
       ...
       <!-- Include the modal -->
       @include('contractservices.modal')
          ...
          document.addEventListener('DOMContentLoaded', function() {
             ...
             document.querySelectorAll('.form-check-input').forEach((checkbox) => {
                checkbox.addEventListener('change', function() {
                   ...
                   fetch('{{ route('contractservices.close_plan') }}', {
                         method: 'POST',
                         headers: {
