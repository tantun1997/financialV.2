    ...
    @section('contents')
       ...
       <div class="mb-3">
          @if ($E_CLOSE_PLAN->status == 'Y')
             <button type="button" class="btn btn-primary" id="addItemButton" data-bs-toggle="modal" data-bs-target="#addModal" data-action="{{ route('potentials.store') }}">
                + เพิ่มรายการ
             </button>
          @endif
       ...
       <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <hr />
          <a href="{{ route('export.data', ['type' => 'potentials']) }}" class="btn btn-success mb-2">Export to
             Excel</a>
       </div>
                   ...
                   <tr>
                      ...
                      <td class="text-center align-middle">
                         <div class="btn-group" role="group">
                            <a href="{{ route('potentials.show', $item->PLAN_ID) }}" type="button" class="btn btn-outline-primary btn-sm" style="white-space: nowrap;">ดูข้อมูล</a>
                            <form action="{{ route('potentials.destroy', $item->PLAN_ID) }}" method="POST" onsubmit="return confirm('Delete?')" class="p-0">
                               @csrf
                               @method('DELETE')
                         ...
                         @if ($item->USAGE_STATUS_ID == 1 && Auth::user()->id == '114000041')
                            <form id="approval-form-{{ $item->PLAN_ID }}" action="{{ route('potentials.approved', $item->PLAN_ID) }}" method="POST">
                               @csrf
                               @method('PATCH')
          ...
          document.addEventListener('DOMContentLoaded', function() {
             ...
             document.querySelectorAll('.form-check-input').forEach((checkbox) => {
                checkbox.addEventListener('change', function() {
                   ...
                   fetch('{{ route('potentials.close_plan') }}', {
                         method: 'POST',
                         headers: {
